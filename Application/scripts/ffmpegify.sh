...
for i in `cat ../tmp/text/audioOrder`
do
...
counter=1
for i in `cat ../tmp/text/imageOrder`
do
...
imageAmount=`cat ../tmp/text/imageOrder | wc -w`
...
then
	ffmpeg -framerate $frameRate -i ../tmp/images/saved/img001.jpg -i ../tmp/audio/concatenatedAudio.wav -vf "scale=750:500,fps=25" -c:v libx264 -pix_fmt yuv420p -y ../tmp/video/tmp.mp4 &> /dev/null
elif [ $imageAmount -eq 2 ] #When there are 2
then
	bash ../tmp/images/saved/ffmpeg_2images.sh
else
	ffmpeg -framerate $frameRate -i ../tmp/images/saved/img%03d.jpg -i ../tmp/audio/concatenatedAudio.wav -vcodec libx264 -pix_fmt yuv420p -vf scale=750:500 -r 25 -max_muxing_queue_size 1024 -y ../tmp/video/tmp.mp4 #&> /dev/null
fi

echo "75" #Percentage

#Overlay text
ffmpeg -i ../tmp/video/tmp.mp4 -filter_complex "drawtext=text='$query':fontcolor=white:fontsize=72:shadowcolor=black:shadowx=2:shadowy=2:x=(w-text_w)/2:y=(h-text_h)/2" -max_muxing_queue_size 1024 -y ../creations/$fileName.mp4 &> /dev/null
